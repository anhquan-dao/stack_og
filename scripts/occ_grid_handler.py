...
class OccupancyGridCallback():
	def __init__(self, baselink_frame, output_shape, output_resolution, input_topic):
		...
		self.og_sub = rospy.Subscriber(self.input_topic, OccupancyGrid, self.callback, queue_size = 1)
	...
	def callback(self, msg):
		...
		self.data_mat = np.array(msg.data, np.uint8).reshape(self.height, self.width)
	...
	def tf_to_baselink_frame(self):
		...
		uncrop_tf_to_baselink_mat = self.get_tf_to_baselink_mat(self.resized_mat, self.sensor_frame, self.baselink)
		crop_mat = self.get_sensor_frame_crop_mat(self.sensor_frame_crop_tf)

		self.tf_to_baselink_mat = np.matmul(uncrop_tf_to_baselink_mat, crop_mat)
		#print(self.tf_to_baselink_mat)
		self.baselink_frame_mat = cv2.warpAffine(self.resized_mat, self.tf_to_baselink_mat[:2, :], self.output_shape)
	...
	def get_sensor_frame_crop_mat(self, crop_tf):
		...
		og_rot_mat          = ros_tf.transformations.quaternion_matrix(twod_quaternion)[:3, :3]

		warp_affine_mat       = og_rot_mat
		warp_affine_mat[0, 2] = self.output_shape[0]/2  + crop_tf[0][0]/self.output_resolution
		warp_affine_mat[1, 2] = self.output_shape[1]/2  + crop_tf[0][1]/self.output_resolution
	...
	def get_tf_to_baselink_mat(self, og_np, sensor_frame, baselink_frame):
		...
		if not sensor_frame == "":
			tf_to_baselink = self.tfBuffer.lookup_transform(sensor_frame, baselink_frame, rospy.Time(0))

			tf_to_baselink_trans, tf_to_baselink_rot = tf_to_baselink.transform.translation, tf_to_baselink.transform.rotation
		else:
			rospy.logerr("Sensor frame for topic <" + self.input_topic + "> is empty")
		'''
		try:
		...
		og_trans_mat[0:2,2]   = np.array([-tf_to_baselink_trans.x, -tf_to_baselink_trans.y])/self.output_resolution
