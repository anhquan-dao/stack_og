...
#----------------------------------------------tf_listener.getLatestCommonTime(sensor_frame_id, BASE_LINK_FRAME)---------------------------------------------
...
def transform_og2currentpose(og_np):
	...
	current_translation, current_rotation    = tf_listener.lookupTransform(BASE_LINK_FRAME, 
																	       FIXED_FRAME, 
																		   tf_listener.getLatestCommonTime(BASE_LINK_FRAME, FIXED_FRAME))
	...
	print(current_warp_affine_mat)
	print(last_warp_affine_mat)
	print(last_to_current_warp_affine_mat)
...
last_current_pose = tf_listener.lookupTransform('/map', 
  												'/base_link', 
												tf_listener.getLatestCommonTime('/map', '/base_link'))

rospy.Subscriber('/map/image_segementation', OccupancyGrid, callback, queue_size=1)
#rospy.Subscriber('/map/abc', OccupancyGrid, callback, queue_size=1)
og_publisher = rospy.Publisher('output', OccupancyGrid, queue_size=1)
