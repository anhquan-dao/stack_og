...
class OccupancyGridStack():
    def __init__(self, fixed_frame="map", base_link="base_link",
		...
		'''
        self.count_map = np.zeros(self.output_shape + (2,), np.float32)
        # Avoid division with zero if no observation is made
        self.count_map[:, :, 1] = 0.0001
        ...
        self.og_publisher = rospy.Publisher(
            "test/output", OccupancyGrid, queue_size=1)

        self.tfBuffer = tf2_ros.Buffer()
        self.tf_listener = tf2_ros.TransformListener(self.tfBuffer)
        self.tf2_ros_exception = (
            tf2_ros.LookupException, tf2_ros.ExtrapolationException)
        ...
        for topic in topics_list:
            self.og_handler_holder.append(OccupancyGridCallback(
                self.base_link, self.output_shape, self.resolution, topic))
    ...
    def tf_stack_to_current_pose(self):
        ...
        current_translation, current_rotation = (
            last_translation, last_rotation)
        ...
        # try:
        current_tf = self.tfBuffer.lookup_transform(
        ...
        last_to_current_warp_affine_mat = np.matmul(
            current_warp_affine_mat, np.linalg.inv(last_warp_affine_mat))
        ...
        last_to_current_warp_affine_mat = np.matmul(np.linalg.inv(
            translate_to_center), last_to_current_warp_affine_mat)
        last_to_current_warp_affine_mat = np.matmul(
            last_to_current_warp_affine_mat, translate_to_center)
        ...
        self.count_map = cv2.warpAffine(self.count_map, self.last_to_current_warp_affine_mat, self.output_shape,
                                        cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=[0, 0.0001])

        self.last_current_pose.transform.translation, self.last_current_pose.transform.rotation = (
            current_translation, current_rotation)
    ...
    def publish_og_msg(self):
        ...
        for og_handler in self.og_handler_holder:
            ...
            self.temp_stack = np.maximum(
                self.temp_stack, og_handler.baselink_frame_mat)
        ...
        #self.current_stack_signed = self.count_map.round().astype(np.int8) - 1
        ...
        self.map_msg.info.origin.position.x = - \
            self.output_size[0]/2  # Unit: Meter
        self.map_msg.info.origin.position.y = - \
            self.output_size[1]/2  # Unit: Meter
    ...
    def BATOLOPU(self, input_submap):
        ...
        self.count_map[:, :, 0][input_submap == 101] += 1
        self.count_map[:, :, 1][input_submap >= 1] += 1

        self.count_prob_mask = 100.0 * \
            (self.count_map[:, :, 0] / self.count_map[:, :, 1])

        self.current_stack_signed = self.count_prob_mask.astype(np.int8)
        self.current_stack_signed[self.count_prob_mask > 100] = 100
        self.current_stack_signed[self.count_prob_mask <=
                                  self.threshold_value] = 0

        # self.current_stack_signed = cv2.threshold(self.count_prob_mask, self.threshold_value, 100, cv2.THRESH_TOZERO)[1].astype(np.int8)
        # self.current_stack_signed[self.count_prob_mask > 100] = 100

        self.current_stack_signed[self.count_map[:, :, 1] < 2] = -1
        self.current_stack_signed[input_submap == 101] = 100
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        if runtime > 1.0/update_frequency:
            rospy.logwarn_throttle(1, "Stack grid did not achieve desired update rate: " + str(
                update_frequency) + ", actually took " + str(runtime))
        else:
